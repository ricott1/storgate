...
<p>ContentHash: {}</p>
			...
			'''.format(query, database[query]['owner'], database[query]['resolver'],database[query]['address'], database[query]['content'], database[query]['contenthash'])
...
def get_resolver_data(query, net='ropsten'):
    ...
    return {'owner' : owner, 'resolver' : resol, 'address' : address, 'content' : content, 'contenthash' : contenthash, 'onion' : onion, 'ipfs' : ipfs, 'swarm' : swarm}
